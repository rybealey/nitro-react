...
export function CanManipulateFurniture(roomSession: IRoomSession, objectId: number, category: number): boolean
{
    ...
    return (IsRoomOwnerNow(roomSession) || (roomSession.controllerLevel >= RoomControllerLevel.GUEST) || HasAnyRoomRights() || IsOwnerOfFurniture(GetRoomEngine().getRoomObject(roomSession.roomId, objectId, category)));
}
